...
\author{Radon Rosborough \\ \hfill \\ \href{https://intuitiveexplanations.com/calculus-bowl/}{https://intuitiveexplanations.com/calculus-bowl/}}
\date{April 2016}
\titlepage
{\linespread{1.5}
\noindent \Tiny Build \thecompilation

\noindent Copyright \textcopyright{} 2016 by Radon Rosborough. This work is licensed under a \href{http://creativecommons.org/licenses/by-nc-nd/4.0/}{Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License}. \par}
\end{frame}
...
\question
{Find the slope of the tangent line to $y = \frac{9x^3 - 27x^2 + 5x - 6}{55x^4 + 11x^3 - 7x^2 + 6x + 3}$ at $x = 0$.}
%CB:IGNORE
{$1$}
{\correct $2$}
{$3$}
...
\question
{If $x = y^2$, $y = z^3$, and $z = w^4$, then what is $\frac{dx}{dy} + \frac{dx}{dz} + \frac{dx}{dw}$?}
...
\question
{Suppose that $t$ is measured in meters and $v(t)$ is measured in seconds. What are the units of $\int v''(t) \,dt$?}
{s}
{m}
{m/s$^2$}
{m/s}
...
\question
{Let $f$ be a continuous function such that $f(x) + f(1 - x) \neq 0$ for all $x$. Evaluate $\int_0^1 \frac{f(x)}{f(x) + f(1 - x)} \,dx$.}
{$0$}
...
\question
{Suppose that $\int_{-3}^4 f(x) \,dx = 3$, $\int_{-1}^7 f(x) \,dx = 7$, and $\int_{-3}^7 f(x) \,dx = 5$. What is $\int_{-1}^4 f(x) \,dx$?}
{\correct $5$}
{$8$}
{$2$}
{$10$}
...
\question
{Find the value of $\sum_{n=0}^\infty \frac{\sin(\frac{n\pi}{2})}{n!}$.}
%CB:IGNORE
{$\cos 1$}
{$\ln 2$}
{\correct $\sin 1$}
...
\question
{Which of the following is \alert{not} a correct solution to the problem $\int 2 \tan x \sec^2 x \,dx$?}
{$\frac{2}{1 + \cos 2x}$}
{$\sec^2 x$}
...
\question
{Suppose that the position of a particle is given by the equation $x = \sin(\pi t)$. Find its velocity when $t = 1/6$, given that $x$ is time and $t$ is position.}
{$\frac{\sqrt{3} \pi}{2}$}
...
\question
{What is the behavior of the expression $\lim_{m \to \infty} \lim_{n \to \infty} \cos^{2n}(m! \pi x)$? Assume $m$ and $n$ are integers.}
%CB:IGNORE
{It is always zero}
{It is always one}
{It does not exist}
...
\question
{Identify the differential equations whose solutions would be underestimated by Euler's method.
...
\question
{Find $\int_\pi^{\pi^2} \frac{\pi^2 \ln(\pi^\pi)}{\theta(\pi + \pi^2)} \,d\theta$.}
...
\question
{Determine the surface area of the solid generated by revolving the curve $y = \sqrt{4 - x^2}$ about the $x$-axis.}
%CB:IGNORE
{$2 \pi$}
{$4 \pi$}
{$\frac{16}{3} \pi$}
...
\question
{Which of the following integrals, when evaluated, gives the area of one of the regions bounded by the curves $y = \cos x$ and $y = \sin x$?}
...
\question
{What is the average value of the function $f(x) = \sqrt{1 - x^2}$ over its domain?}
{\correct $\pi/4$}
{$\pi/6$}
{$3/4$}
{$\sqrt{3}/2$}
...
\question
{Evaluate $\lim_{x \to \infty} \frac{\sum_{n=3}^{9} nx^n}{\sqrt[3]{\sum_{n=9}^{27} \frac{x^n}{n}}}$.}
%CB:IGNORE
{$0$}
{$3$}
{$9$}
...
\question
{Evaluate $\frac{d}{dx} \big[\ln \ln \ln \ln x\big]$ at $x = e^{(e^e)}$.}
{$e^{-e - 1}$}
{$e^{e^e + e + 1}$}
{\correct $e^{-e^e - e - 1}$}
{$e^{-e^{-e - 1} - e}$}
...
\question
{Taken collectively, how many different intervals of convergence do the following series have?
\begin{align*}
...
\question
{How many points of inflection can a polynomial of degree $7$ have, at most?}
%CB:IGNORE
{$3$}
{\correct $5$}
{$6$}
...
\question
{The power rule states that $\frac{d}{dx} \big[x^n\big] = nx^{n-1}$. In which of the following situations does the power rule \alert{not} hold?
{\renewcommand{\theenumi}{\Roman{enumi}}
\begin{enumerate}
\item $x = 0$ and $n < 1$
\item $x = 0$ and $n = 1$
...
\question
{If the arc length of the curve given by $x(t) = \cos \sqrt{t}$ and $y(t) = \sin \sqrt{t}$ from $t = 0$ to $t = T$ is $6\pi$, then what is $T$?}
%CB:IGNORE
{$\sqrt{3}$}
{$\sqrt{6}$}
{$3$}
...
\question
{Evaluate $\frac{d}{dx} \Big[x + x^2 + x^3 + \cdots + x^{100}\Big]$ at $x = 1$.}
%CB:IGNORE
{$99$}
{$100$}
{\correct $5050$}
...
\question
{What is the rate of change of the length of the hypotenuse of an isosceles right triangle with respect to the length of one of its legs?}
%CB:IGNORE
{$0$}
{$1$}
{\correct $\sqrt{2}$}
...
\question
{What is $\frac{d}{dA} \big[ABRACADABRA\big]$ when $A = 1$, $B = 2$, $C = 3$, and $D = 4$?}
%CB:IGNORE
{$5R^2$}
{$48R^2$}
{$120R^2$}
...
\question
{Determine the value of $\frac{d}{dx} \Big[|\sin x| + |\cos x| + |\tan x|\Big]$ at $x = \frac{\pi}{4}$.}
%CB:IGNORE
{$1$}
{$\sqrt{2}$}
{$1 + \frac{\sqrt{2}}{2}$}
...
\question
{Suppose that the radius of a sphere is $1$ inch, and its volume is increasing at a rate of $1$ cubic inch per minute. How fast is its surface area increasing?}
%CB:IGNORE
{$1$ in$^2$/min}
{\correct $2$ in$^2$/min}
{$2\pi$ in$^2$/min}
...
\question
{For what value of $p$ is the expression $\lim_{n \to \infty} \frac{\sqrt{1} + \sqrt{2} + \sqrt{3} + \cdots + \sqrt{n}}{n^p}$ finite and nonzero?}
%CB:IGNORE
{$1/2$}
{$1$}
{\correct $3/2$}
...
\question
{Consider a collection of spheres with radii given by $\left\{1, \frac{1}{2^p}, \frac{1}{3^p}, \ldots\right\}$. If the total volume is finite but the total surface area is infinite, then what values of $p$ are possible?}
{$2 \leq p < 3$}
...
\question
{Suppose that $f$ is a function defined on an interval $I$. Which of the following statements \alert{must} be true?
{\renewcommand{\theenumi}{\Roman{enumi}}
\begin{enumerate}
