...
\renewcommand{\vec}[1]{\ensuremath{\mathbf{#1}}} % name of vector
\newcommand{\mat}[1]{\ensuremath{#1}} % name of matrix
\newcommand{\R}[1][]{\ensuremath{\mathbb{R}^{#1}}} % set of all n-tuples of real numbers
\newcommand{\by}{\ensuremath{\times}} % $2 \by 3$ matrix => $2 \times 3$ matrix
\newcommand{\MAT}[2][r]{\ensuremath{\begin{bmatrix*}[#1]#2\end{bmatrix*}}} % matrix
\NewEnviron{matr}[1][r]{\ensuremath{\begin{bmatrix*}[#1]\BODY\end{bmatrix*}}} % matrix
\DeclareMathOperator{\OpSpan}{Span} % span of a set of vectors
\newcommand{\Span}[1]{\ensuremath{\OpSpan\{#1\}}} % span of a set of vectors
\newcommand{\set}[1]{\ensuremath{\{#1\}}} % set
\newcommand{\inv}{\ensuremath{^{-1}}} % inverse of a matrix
\newcommand{\trans}{\ensuremath{^T}} % transpose of a matrix
\DeclareMathOperator{\adj}{adj} % adjugate (classical adjoint) of a matrix
\DeclareMathOperator{\Nul}{Nul} % null space of a matrix
\DeclareMathOperator{\Col}{Col} % column space of a matrix
\DeclareMathOperator{\Row}{Row} % row space of a matrix
\newcommand{\basis}[1]{\ensuremath{\mathcal{#1}}} % basis for a vector space
\DeclareMathOperator{\rank}{rank} % rank of a matrix
\newcommand{\eq}{\sim} % row equivalence
\newcommand{\from}{\leftarrow} % used in names of change-of-coordinates matrices
\newcommand{\coord}[2]{\ensuremath{\left[#1\right]_{#2}}} % coordinates of a vector relative to a basis
\newcommand{\chcoordr}[1]{\ensuremath{P_{#1}}} % change-of-coordinates matrix from a basis to the standard basis
\newcommand{\chcoord}[2]{\ensuremath{P_{#1 \from #2}}} % change-of-coordinates matrix between two bases
\newcommand{\transmat}[2]{\coord{#1}{#2}} % matrix for a linear transformation relative to a basis
\newcommand{\transmatb}[3]{\coord{#1}{#2 \from #3}} % matrix for a linear transformation relative to two bases
\newcommand{\seq}[3][]{\ifemptyarg{#1}{\ensuremath{\{{#2}_{#3}\}}}{\ensuremath{\{{#2}_{#3}^{(#1)}\}}}} % sequence
\newcommand{\Signals}{\ensuremath{\mathbb{S}}} % set of all signals
\newcommand{\many}[2][n]{\ensuremath{{#2}_1, \ldots, {#2}_{#1}}} % \many[k]{\vec v} => $\vec v_1, \ldots, \vec v_k$
...
\newcommand{\comp}{^\bot} % orthogonal complement
\newcommand{\norm}[1]{\left|\left|#1\right|\right|} % magnitude of a vector
\DeclareMathOperator{\proj}{proj} % orthogonal projection of a vector onto a subspace
...
\titlepage{Linear Algebra Summary Sheet}{linear-algebra-summary-sheet}{This document is a concise collection of many of the important theorems of linear algebra, organized so that making connections between different concepts and theorems is as easy as possible. The material here is primarily based on the textbook I used in my Linear Algebra class (\textit{Linear Algebra and Its Applications} by David C. Lay, fourth edition).}
...
Let $W$ be a subspace of \R[m], let \basis U = \set{\many{\vec u}} be a basis for $W$, let $\mat U = \MAT{ \vec u_1 & \cdots & \vec u_n }$ be an $m \by n$ matrix, and let \vec y be a vector in \R[m]. Then:
\begin{enumerate}
\item There is a unique representation of \vec y in the form $\vec y = \vec{\hat y} + \vec z$, where \vec{\hat y} is in $W$ and \vec z is in $W\comp$. Typically \vec{\hat y} is denoted by $\proj_W \vec y$.
\item The closest point in $W$ to \vec y is \vec{\hat y}, i.e. $\norm{\vec y - \vec{\hat y}} < \norm{\vec y - \vec v}$ if $\vec v \neq \vec{\hat y}$.
\item The equation $\mat U\trans \mat U \vec x = \mat U\trans \vec y$ has a unique solution, and $\vec{\hat y} = \mat \vec x$.
\item If \basis U is an orthogonal basis for $W$, then
\begin{equation*}
\vec{\hat y} = \frac{\vec y \dot \vec u_1}{\vec u_1 \dot \vec u_1} \vec u_1 + \cdots + \frac{\vec y \dot \vec u_n}{\vec u_n \dot \vec u_n} \vec u_n.
\end{equation*}
\item If \basis U is an orthonormal basis for $W$, i.e. \mat U is an orthogonal matrix, then
\begin{equation*}
\vec{\hat y} = (\vec y \dot \vec u_1) \vec u_1 + \cdots + (\vec y \dot \vec u_n) \vec u_n = \mat U \mat U\trans \vec y.
\end{equation*}
\item If \basis U is an orthonormal basis for \R[m], i.e. \mat U is a square orthogonal matrix, then $\vec{\hat y} = \vec y$.
\item Let
\begin{align*}
\vec v_1 &= \vec u_1, \\
...
\end{align*}
This method of obtaining an orthogonal basis for $W$ is called the Gram-Schmidt process.
\end{enumerate}
\end{thm}

\begin{thm}[Properties of Orthogonal Matrices] \hfill
\begin{enumerate}
\item Let \mat U be a matrix. Then \mat U has orthonormal columns if and only if $\mat U \mat U\trans = \mat I$, and \mat U has orthonormal rows if and only if $\mat U\trans \mat U = \mat I$.
\item Let \mat U be a square matrix. Then \mat U has orthonormal columns if and only if it has orthonormal rows.
\item Let \mat U be an $m \by n$ orthogonal matrix, and let \vec x and \vec y be vectors in \R[m]. Then \mat U preserves lengths, angles, and orthogonality. That is:
\begin{enumerate}
\item $\norm{\mat U \vec x} = \norm{\vec x}$.
\item $(\mat U \vec x) \dot (\mat U \vec y) = \vec x \dot \vec y$.
\item $(\mat U \vec x) \dot (\mat U \vec y) = 0$ if and only if $\vec x \dot \vec y = 0$.
\end{enumerate}
\item Let \mat A be an $m \by n$ matrix with linearly independent columns, and let $\mat U = \MAT{ \vec u_1 & \cdots & \vec u_n }$ be the $m \by n$ orthogonal matrix obtained by finding an orthogonal basis for $\Col \mat A$ using the Gram-Schmidt process, normalizing the vectors, and using them as the columns of \mat U. Let \mat R be the $n \by n$ matrix equal to $\mat Q\trans \mat A$. Also, for $1 \leq i \leq n$, if the $(i, i)$ entry of \mat R is negative, multiply the $i$th row of \mat R and the $i$th column of \mat U by $-1$. Then $\mat A = \mat P \mat Q$, and \mat R is an upper triangular invertible matrix with positive entries on its diagonal.
\end{enumerate}
\end{thm}
...
\begin{enumerate}
\item $\norm{\vec u + \vec v}^2 = \norm{\vec u}^2 + \norm{\vec v}^2$ if and only if $\vec u \dot \vec v = 0$.
